...
namespace DVUnityUtilities
{
    ...
    {
        public static void SetCharacterOrRigidbodyOrTransformPosition(Transform go, Vector3 pos)
        {
            if (go.TryGetComponent<Rigidbody>(out var rb))
            {
                rb.position = pos;
            }
            else if (go.TryGetComponent<CharacterController>(out var c))
            {
                bool c_enabled = c.enabled;
        ...
        public static bool TryGetComponentInChildren<T>(this Transform go, bool includeInactive, out T res)
        {
            return go.gameObject.TryGetComponentInChildren<T>(includeInactive, out res);
        }

        public static bool TryGetComponentInChildren<T>(this GameObject go, bool includeInactive, out T res)
        {
            res = go.GetComponentInChildren<T>();

            if (res is Object t) return t;
            else return res != null;
        }

        public static void ClearSources(this UnityEngine.Animations.ParentConstraint pc)
        {
            while (pc.sourceCount > 0)
            {
                pc.RemoveSource(0);
            }
        }

        public static void ValidateComponentOrGet<T>(GameObject go, ref T val) where T : Component
        {
            if (!val) val = go.GetComponent<T>();
        }

        public static void SetEnableIfNot(Behaviour component, bool v)
        {
            if (component.enabled != v) component.enabled = v;
        }

        public static void DeactivateChilds(GameObject go)
        {
        ...
        public static void SetActiveIfNot(this GameObject go, bool isActive)
        {
        ...
        public static List<T> FindObjectsWithComponentOfType<T>(bool includeInactive)
        {
            List<T> interfaces = new List<T>();
            GameObject[] rootGameObjects = Object.FindObjectsOfType<GameObject>(includeInactive);
            ...
            return interfaces.Distinct().ToList(); // todo: distinct???
        }

        public static T FindObjectWithComponentOfType<T>(bool includeInactive) where T : class
        {
            return FindObjectWithComponentOfType(typeof(T), includeInactive) as T;
        }

        public static object FindObjectWithComponentOfType(System.Type t, bool includeInactive)
        {
            GameObject[] rootGameObjects = Object.FindObjectsOfType<GameObject>(includeInactive);

            foreach (var rootGameObject in rootGameObjects)
            {
                object childrenInterface = rootGameObject.GetComponentInChildren(t);
        ...
        public static void SetEntityColliderStatus(Transform transform, bool isEnable, bool includeInactive)
        {
            var cols = transform.GetComponentsInChildren<Collider>(includeInactive);
        ...
        public static void DestroyGameObjectsAndClear(this List<GameObject> objects)
        {
            objects.ForEach(x =>
            {
                if (x) GameObject.Destroy(x.gameObject);
            });
            objects.Clear();
        }
        ...
        public static bool IsFlipped(Vector3 up1, Vector3 up2, float minAngleForFlip)
        {
            float angle = Vector3.Angle(up1, up2);
            return angle >= minAngleForFlip;
        }

        public static void SetPhysicMaterial(GameObject go, PhysicMaterial pm)
        {
